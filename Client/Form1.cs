...
namespace Client
{
    public partial class Form1 : Form
    {
        ...
        private async void Mstrip_connectItem_Click(object sender, EventArgs e)
        {
            ...
            // перев≥рка п≥дключенн€ до сервера
            try
            {
                str = await UDPMessanges.UDPGetMessangeAsync(UDPConnecting.udpClient, new IPEndPoint(IPAddress.Any, 0)); ;//отриманн€ пов≥домленн€ з PlayerTag гравц€ (Player1 or Player2)
            }
            catch (Exception)
            {
            ...
            if (Player.ID == 1)
            {
                ...
                try
                {
                    s = await UDPMessanges.UDPGetMessangeAsync(UDPConnecting.udpClient, new IPEndPoint(IPAddress.Any, 0));//€кщо гравцю було присвоенно PlayerTag Player1 чекаемо п≥дключенн€ другого гравц€
                }
                catch (Exception)
                {
                    Mstrip_connectItem.Enabled = true;
        ...
        // в≥дстежуЇ натиск клав≥ш Up, Down, Left, Right, Space
        ...
        //перемiщае випущен≥ снар€ди по полю
        void Foo()
        {
